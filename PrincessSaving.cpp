...
void PrincessSaving::Enter(Agent* target)
{
	if (target->GetPath().empty())//if no current path
	{
		Node* start = target->GetNodeMap().GetClosestNode(target->GetPosition());
		Node* end = target->GetNodeMap().GetClosestNode(target->GetNodeMap().GetPrincess()->GetPosition());
		std::vector<Node*> path = target->GetNodeMap().DijkstrasSearch(start, end->previous);//this time we're aiming to stand in front of the princess
		target->GoToNode(start, end->previous);
	}
}

void PrincessSaving::Update(Agent* target, float deltaTime)
{
	int leeway = 1;
	...
	else //we've either arrived or overshot it
	{
		...
		if (target->GetCurrentIndex() == target->GetPath().size())//if we've reached the last node in the list
		{
			...
			target->GetNodeMap().GetPrincess()->SavePrincess();
		}
