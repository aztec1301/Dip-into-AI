...
void GoToPost::Update(Agent* target, float deltaTime)
{
	if (target->GetPath().empty())//if no current path
	{
		Node* start = target->GetNodeMap().GetClosestNode(target->GetPosition());
		Node* end = target->GetNodeMap().GetClosestNode(target->GetGemInfo()[target->GetGemToGuard()].GetPosition());
		std::vector<Node*> path = target->GetNodeMap().DijkstrasSearch(start, end->previous);
